...
const { db } = require('../db');
const { orders, users, menuItems, bonusTransactions, partners, userBonusesByPartner } = require('../db/schema');
...
async function showAdminPanel(ctx) {
  ...
  ctx.reply('🔐 *Admin Panel* - Ընտրիր գործողությունը:', { parse_mode: 'Markdown', ...adminMenu() });
}
...
async function showStats(ctx) {
  ...
  const keyboard = Markup.inlineKeyboard([[Markup.button.callback('🔙 Հետ', 'back_to_admin')]]);
  await ctx.reply(text, { parse_mode: 'Markdown', ...keyboard });
}
